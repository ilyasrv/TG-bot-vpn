...
def profile(message):
 ...
 Инлайн кнопки КУПИТЬ 
@bot.message_handler(func=lambda message: message.text == "Купить")
def buy(message):
    markup = types.InlineKeyboardMarkup()
    month_button = types.InlineKeyboardButton("1 месяц (100 руб.)", callback_data='1_month')
    three_months_button = types.InlineKeyboardButton("3 месяца (250 руб.)", callback_data='3_months')
    six_months_button = types.InlineKeyboardButton("6 месяцев (450 руб.)", callback_data='6_months')
    twelve_months_button = types.InlineKeyboardButton("12 месяцев (800 руб.)", callback_data='12_months')
    markup.row(month_button, three_months_button)
    markup.row(six_months_button, twelve_months_button)
...
def buy(message):
    ...
    markup.add(three_months_button)
    markup.add(six_months_button) 
    markup.add(twelve_months_button) 
